...
function TextBlock({ content }: { content: TextContent }) { return <section id="text" className="bg-[#0b0d12] py-20 text-white"><Container><div className="mx-auto max-w-3xl text-center"><h2 className="text-3xl font-extrabold sm:text-4xl">{content.title}</h2><LinkifiedText text={content.body} className="mt-6 whitespace-pre-line text-base leading-relaxed text-white/70" /></div></Container></section>; }
...
function CustomSection({ content, settings }: { content: CustomContent; settings: SiteSettingsRecord }) { return <section className="bg-[#0b0d12] py-20 text-white"><Container><div className="mx-auto max-w-4xl"><div className="mb-12 text-center"><h1 className="text-3xl font-extrabold sm:text-4xl">{content.title}</h1>{content.intro ? <LinkifiedText text={content.intro} className="mx-auto mt-4 max-w-2xl whitespace-pre-line text-white/60" /> : null}</div><div className="space-y-5">{content.items?.map((item, index) => <article key={index} className="rounded-2xl border border-white/10 bg-white/[0.03] p-6"><div className="mb-3 h-1 w-12 rounded-full" style={{ backgroundColor: settings.accentColor }} /><h2 className="text-xl font-bold">{item.heading}</h2>{item.comment ? <LinkifiedText text={item.comment} className="mt-3 whitespace-pre-line leading-relaxed text-white/65" /> : null}</article>)}</div></div></Container></section>; }
