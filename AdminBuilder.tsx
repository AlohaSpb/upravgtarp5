...
import { SECTION_ICONS, SECTION_LABELS, SECTION_TYPES } from "@/lib/section-types";
...
export function AdminBuilder() {
  ...
  function startEditing(section: SectionRecord) {
  ...
  if (loading || !settings || !settingsDraft) {
    return (
      <div className="grid min-h-screen place-items-center bg-[#0b0d12] text-white/60">
      ...
      <div className="sticky top-0 z-40 border-b border-white/10 bg-[#0b0d12]/95 px-6 py-4 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between">
          <div>
            ...
            <p className="text-xs text-white/40">Управление кадров — редактирование информационного портала</p>
          </div>
          <div className="flex items-center gap-3">
            {toast ? <span className="text-xs font-semibold text-green-400">{toast}</span> : null}
            <a href="/" target="_blank" rel="noreferrer">
              <GhostButton type="button">Открыть сайт ↗</GhostButton>
            </a>
          ...
          <Panel title="Настройки сайта">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Эмодзи-логотип">
                <TextInput
                  value={settingsDraft.logoEmoji}
                  onChange={(e) => setSettingsDraft({ ...settingsDraft, logoEmoji: e.target.value })}
                />
              </Field>
            ...
            <Field label="Новый пароль администратора" hint="Оставьте пустым, чтобы не менять">
              <TextInput
                type="password"
                value={newPassword}
            ...
            <PrimaryButton type="button" onClick={saveSettings} disabled={savingSettings} className="w-full">
          ...
          <Panel title="Добавить раздел">
            <Field label="Тип раздела">
              <select
                ...
                className="w-full rounded-lg border border-white/15 bg-black/30 px-3.5 py-2.5 text-sm text-white outline-none"
              >
            ...
            <PrimaryButton type="button" onClick={handleAddSection} className="w-full">
              + Добавить на сайт
            </PrimaryButton>
          </Panel>
        </div>
        ...
        <div className="space-y-4">
          {sections.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-white/15 p-10 text-center text-white/40">
              Разделов пока нет. Добавьте первый слева.
            </div>
          ) : null}
              ...
              <div key={section.id} className="rounded-2xl border border-white/10 bg-white/[0.03]">
                <div className="flex items-center gap-3 p-4">
                  ...
                  {isEditing ? (
                    ...
                    <GhostButton type="button" onClick={() => startEditing(section)}>
                      Редактировать
                    </GhostButton>
                  )}
                  <DangerButton type="button" onClick={() => handleDelete(section.id)}>
                    Удалить
                  </DangerButton>
                </div>
                ...
                {isEditing && draftContent ? (
                  <div className="border-t border-white/10 p-5">
                    ...
                    <div className="mt-4 flex gap-3">
                      <PrimaryButton type="button" onClick={() => saveEditing(section.id)}>
                        Сохранить раздел
                      </PrimaryButton>
                      <GhostButton type="button" onClick={cancelEditing}>
                        Отмена
